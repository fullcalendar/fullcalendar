...
$.extend(ListView.prototype, {
	...
	render: function(rowCnt, colCnt, dayNumbersVisible) {
		...
		this.dayNumbersVisible = false;//dayNumbersVisible;
		this.weekNumbersVisible = true;//this.opt('weekNumbers');
	...
	numberIntroHtml: function(row) {
		if (this.weekNumbersVisible) {
			return '' +
				'<td class="fc-week-number" ' + this.weekNumberStyleAttr() + '>' +
					'<span>' + // needed for matchCellWidths
						moment(this.cellToDate(row,0)).format('ddd MMM D, YYYY') +
						//this.calendar.calculateWeekNumber(this.cellToDate(row, 0)) +
					'</span>' +
				'</td>';
		}
	...
	numberCellHtml: function(row, col, date) {
		...
		return '' +
			'<td class="' + classes.join(' ') + '" data-date="' + date.format() + '">bbb' +
				date.date() +
			'</td>';
	},
