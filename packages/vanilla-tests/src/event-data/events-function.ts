describe('events as a function', () => {
  ...
  it('requests the correct dates when days at the start/end of the month are hidden', (done) => {
    initCalendar({
      ...
      events(info, callback) {
        expect(info.start).toEqualDate('2013-06-03T00:00:00Z')
        expect(info.end).toEqualDate('2013-06-29T00:00:00Z')
  ...
  it('does not request dates excluded by showNonCurrentDates:false', (done) => {
    initCalendar({
      ...
      events(info) {
        expect(info.start).toEqualDate('2013-06-01T00:00:00Z')
        expect(info.end).toEqualDate('2013-07-01T00:00:00Z')
        done()
      },
    })
  })
  ...
  it('requests a timed range when slotMinTime is negative', (done) => {
    initCalendar({
      ...
      slotMinTime: { hours: -2 },
      events(info) {
        expect(info.start).toEqualDate('2017-06-03T22:00:00Z')
        expect(info.end).toEqualDate('2017-06-11T00:00:00Z')
        done()
      },
    })
  })
  ...
  it('requests a timed range when slotMaxTime exceeds 24 hours', (done) => {
    initCalendar({
      ...
      slotMaxTime: '26:00',
      events(info) {
        expect(info.start).toEqualDate('2017-06-04T00:00:00Z')
        expect(info.end).toEqualDate('2017-06-11T02:00:00Z')
        done()
      },
    })
  })
